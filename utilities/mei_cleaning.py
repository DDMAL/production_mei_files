...
parser.add_argument(
    ...
    type=str,
    nargs="?",
    help="Path to mei file for cleaning. If a directory, cleans all mei files in the directory.",
    action="store",
)
parser.add_argument(
    "--remove_unreferenced_bounding_boxes",
    action="store_true",
    help="If flagged, removes zones/bounding boxes that are defined but not referenced anywhere in the body.",
)
parser.add_argument(
    "--remove_identical_duplicates",
    action="store_true",
    help="If flagged, removes duplicate zones/bounding boxes and duplicate objects that reference those bounding boxes.",
)
parser.add_argument(
    "--raise_nonidentical_duplicates",
    action="store_true",
    help="Find and record instances where duplicate zones/bounding boxes are referenced by different, non-identical objects.",
)
parser.add_argument(
    "--destination_path",
    action="store",
    ...
    type=str,
    nargs="?",
    help="If provided, the cleaned file is save here. If omitted, file is save to mei_path location. If mei_path is a directory, this should also be a directory.",
)
parser.add_argument(
    "--report_file",
    action="store",
...
class MEIFileCleaner:
    ...
    def get_elements_with_duplicate_references(self, mei):
        ...
        layer = mei.find(
            f"./{MEINS}music/{MEINS}body/{MEINS}mdiv/{MEINS}score/{MEINS}section/{MEINS}staff/{MEINS}layer"
        )
        duplicate_references_list = []
        for dup_zone_pair in duplicate_zones:
            elems = [layer.find(f".//*[@facs='{dup}']") for dup in dup_zone_pair]
            parent_elems = [
                layer.find(f".//*[@facs='{dup}']/..") for dup in dup_zone_pair
            ]
            dup_ref_list = [
                {
                    "element": elems[0],
                    "bb_id": dup_zone_pair[0],
                    "parent": parent_elems[0],
                },
                {
                    "element": elems[1],
                    "bb_id": dup_zone_pair[1],
                    "parent": parent_elems[1],
                },
            ]
    ...
    def check_element_identity(self, elem1, elem2):
        elem_attribs = [elem1.attrib.copy(), elem2.attrib.copy()]
    ...
    def handle_referenced_duplicates(self, mei):
        ...
        for dup_ref in dup_ref_list:
            identical = self.check_element_identity(
                dup_ref[0]["element"], dup_ref[1]["element"]
            )
            if identical:
                if self.remove_identical_duplicates:
                    surface = mei.find(f"{MEINS}music/{MEINS}facsimile/{MEINS}surface")
                    self.delete_element_and_referenced_zone(
                        surface=surface,
        ...
        if self.raise_nonidentical_duplicates:
            if self.report_file:
                print(
                    f"Non-identical duplicates checked and raised in {self.report_file}"
                )
        return None
